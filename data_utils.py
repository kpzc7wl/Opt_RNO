...
from utils import TorchQuantileTransformer, UnitTransformer, PointWiseUnitTransformer, LogTransformer, MultipleTensors, MinMaxTransformer, micro_obj_batch, drone_obj_batch
from models.cgpt_geo import RNO
from models.mmgpt import GNOT



def get_dataset(args):
    if args.dataset == "micro2d_sens_100x12":
        train_path = './data/microreactor_sens_IO_100x12_train.pkl'
        test_path = './data/microreactor_sens_IO_100x12_test.pkl'
        mask_dim = 1
        sens_dim = 1
        args.objective = micro_obj_batch
    elif args.dataset == "micro2d_sens_30x40":
        train_path = './data/microreactor_sens_IO_30x40_train.pkl'
        test_path = './data/microreactor_sens_IO_30x40_test.pkl'
        mask_dim = 1
        sens_dim = 1
        args.objective = micro_obj_batch
    elif args.dataset == 'shape2d_sens':
        train_path = './data/shape_sens_30x20_train.pkl'
        test_path = './data/shape_sens_30x20_test.pkl'
        mask_dim = 2
        sens_dim = 2
    elif args.dataset == 'inductor2d_sens':
        train_path = './data/inductor_sens_100x10_train.pkl'
        test_path = './data/inductor_sens_100x10_test.pkl'
        mask_dim = 4
        sens_dim = 2
    elif args.dataset == 'drone3d_sens':
        # train_path = './data/drone_sens_40x20_train.pkl'
        # test_path = './data/drone_sens_40x20_test.pkl'
        train_path = './data/drone_sens_10x20_train.pkl'
        test_path = './data/drone_sens_10x20_test.pkl'
        mask_dim = 3
        sens_dim = 1
        args.objective = drone_obj_batch
    else:
    ...
    n_traj, n_step = re.findall(r"\d+", train_path)
    args.n_traj, args.n_step = int(n_traj), int(n_step)

    DSet = MaskDataset
    train_dataset = DSet(train_path, name=args.dataset, train=True, train_num=args.train_num,
    ...
    test_dataset = DSet(test_path, name=args.dataset, train=False, test_num=args.test_num,
                              sample_rate=args.sample_rate,
                              n_step=args.n_step,
                              sort_data=args.sort_data,
                              normalize_y=args.use_normalizer,
    ...
    # if sens_dim > 0: args.dataset_config['sens_dim'] = sens_dim
...
def get_model(args):
    ...
    if 'sens_dim' in args.dataset_config: 
        output_size -= args.dataset_config['sens_dim']
        
    if args.model_name == "RNO":
        branch_sizes = [trunk_size, output_size, trunk_size] # dim of (x_ref, y_ref, dx)
        # trunk_size = args.space_dim  # used before Sobolev Training, depreciated

        return RNO(trunk_size=trunk_size, branch_sizes=branch_sizes, output_size=output_size,
                     n_layers=args.n_layers, n_hidden=args.n_hidden, n_head=args.n_head,
                     gamma=args.gamma, n_slice=args.n_slice, modes=args.modes,
                     attn_type=args.attn_type, ffn_dropout=args.ffn_dropout,
                     attn_dropout=args.attn_dropout, mlp_layers=args.mlp_layers, 
                     act=args.act,horiz_fourier_dim=args.hfourier_dim,space_dim=args.space_dim,
                     n_experts=args.n_experts, n_inner=args.n_inner)

    elif args.model_name == "GNOT":
        # branch_sizes = [trunk_size, output_size, trunk_size]
        branch_sizes = []
        # trunk_size = args.space_dim
        ...
        return GNOT(trunk_size=trunk_size, branch_sizes=branch_sizes, output_size=output_size,n_layers=args.n_layers, n_hidden=args.n_hidden, n_head=args.n_head,attn_type=args.attn_type, ffn_dropout=args.ffn_dropout, attn_dropout=args.attn_dropout, mlp_layers=args.mlp_layers, act=args.act,horiz_fourier_dim=args.hfourier_dim,space_dim=args.space_dim,n_experts=args.n_experts, n_inner=args.n_inner)
...
def remove_padding(padded_tensor):
  """
  Removes the padding zeros in the dimension 1 of a tensor of shape (B, M, D) to have a list of B tensors of shape (M_i, D), where M_i is varying.

  Args:
    tensor: The tensor to remove the padding zeros from.

  Returns:
    A list of tensors, each of shape (M_i, D).
  """

  # Get the shape of the tensor.
  bs, m = padded_tensor.shape[0], padded_tensor.shape[1]

  # Iterate over the batch dimension.
  tensors = []
  for i in range(bs):
    # Find the start and end indices of the non-zero elements in the tensor.
    # for j in range(m)[::-1]:
    #     if torch.all(tensor[i, j] == 0, dim=0): continue
    #     end_indices = j
    #     break
    end_indices = torch.where((padded_tensor[i] > 0).sum(-1) > 0)[0][-1]
    tensors.append(padded_tensor[i, :end_indices+1])
...
class MIODataset(DGLDataset):
    def __init__(self, data_path, name=' ', train=True, test=False, train_num=None, test_num=None, 
                 sample_rate=1, use_cache=True, n_step=10,
                 normalize_y=False, y_normalizer=None, x_normalizer=None, up_normalizer=None, normalize_x=False,sort_data=False):
        ...
        if not os.path.exists(self.cached_path):
            ...
            # data_all = [d for i, d in enumerate(data_all) if i % (2 * n_step) >= n_step] # drop the data of one of load cases
            ...
            # # Downsampling data by sample_rate
            # if self.sample_rate > 1:
            #     print('Downsampling to {} samples'.format(len(self.data_list)))
            #     self.data_list = self.data_list[::self.sample_rate]
            #     # Downsample by pairs
            #     # sample_idx = np.arange(0, len(self.data_list)//2, self.sample_rate)
            #     # sample_idx = sample_idx * 2
            #     # sample_idx = np.concatenate([sample_idx, sample_idx + 1])
            #     # sample_idx = np.sort(sample_idx)
            #     # self.data_list = [self.data_list[idx] for idx in sample_idx]
    ...
    def process(self):
        ...
        for i in range(len(self)):            
            ...
            x, y = x[::self.sample_rate], y[::self.sample_rate] # downsample mesh grids
            g = dgl.DGLGraph()
            ...
            self.graphs.append(g)
            if u_p is not None:
                up = torch.from_numpy(np.array(u_p)).float()
                self.u_p.append(up) # global input parameters     
        ...
        if len(self.u_p) == 0:
            self.u_p = torch.zeros([len(self)])  # pad values, tensor of 0, not list
        ...
        # self.u_p = torch.stack(self.u_p)
        # self.u_p = pad_sequence([u for u in self.u_p]).permute(1, 0)
        self.x_max = self.graphs[0].ndata['x'].max(dim=0).values
        self.x_min = self.graphs[0].ndata['x'].min(dim=0).values

        #### normalize_y
        if self.normalize_y != 'none':
            self.normalize_y_func()
        if self.normalize_x != 'none':
            self.normalize_x_func()
        if isinstance(self.inputs_f, MultipleTensors):
            self.__normalize_f()
    ...
    def normalize_x_func(self):
        if self.x_normalizer is None:
            print(f'Input features are normalized using {self.normalize_x} transformer')
            x_feats_all = torch.cat([g.ndata["x"] for g in self.graphs],dim=0)
    ...
    def __normalize_f(self):
        if self.f_normalizer is None:
            if self.x_normalizer is not None:
                self.f_normalizer = self.x_normalizer
            else:
                raise NotImplementedError
        

        inputs_f = []
        for fs in self.inputs_f:
            fs = [self.f_normalizer.transform(f, inverse=False) for f in fs]
            fs = MultipleTensors(fs)
            inputs_f.append(fs)
        self.inputs_f = inputs_f

        # self.u_p = torch.cat([self.x_normalizer.transform(centers, inverse=False), 
        #                       rads / self.x_normalizer.std.mean()
        #                     ],
        #                     dim=-1).squeeze().view(self.data_len, -1)


    def __update_dataset_config(self):
        self.config = {
            'input_dim': self.graphs[0].ndata['x'].shape[1],
            'theta_dim': self.u_p.shape[1] if isinstance(self.u_p, list) else 0,
            'output_dim': self.graphs[0].ndata['y'].shape[1],
            'branch_sizes': [x.shape[1] for x in self.inputs_f[0]] if isinstance(self.inputs_f, MultipleTensors) else 0,
        }        
        return
...
'''
    X data: N x (space_dim + input_function_dim + mask_dim).
    Mask dimension will not be tranformed.
'''
class MaskDataset(MIODataset):
    def __init__(self, *args, mask_dim=0, sens_dim=0, **kwargs):
        self.mask_dim = mask_dim
        self.sens_dim = sens_dim
        super().__init__(*args, **kwargs)    
        self.config['mask_dim'] = mask_dim
        self.config['sens_dim'] = sens_dim
    
    def normalize_x_func(self):
        if self.x_normalizer is None:
            print(f'Input features are normalized using {self.normalize_x} transformer')
            x_feats_all = torch.cat([g.ndata["x"][:, :-self.mask_dim] for g in self.graphs],dim=0)
            ...
            # elif self.normalize_x == 'minmaxlog':
            #     self.x_normalizer = MinMaxTransformer(x_feats_all, log=True)
            #     # self.up_normalizer = MinMaxTransformer(self.u_p)
            #     print(self.x_normalizer.max, self.x_normalizer.min)
        ...
        for g in self.graphs:
            x_ = self.x_normalizer.transform(g.ndata['x'][:, :-self.mask_dim], inverse=False)
            g.ndata['x'] = torch.cat([x_, g.ndata['x'][:, -self.mask_dim:]], dim=-1)
        # self.u_p = self.up_normalizer.transform(self.u_p, inverse=False)

        
    def normalize_y_func(self):
        if self.y_normalizer is None:
            y_feats_all = torch.cat([g.ndata['y'][:, :-self.sens_dim] for g in self.graphs],dim=0)
            if self.normalize_y == 'unit':
                self.y_normalizer = UnitTransformer(y_feats_all)
                print('Target features are normalized using unit transformer')
                print(self.y_normalizer.mean, self.y_normalizer.std)
            ...
            # elif self.normalize_y == 'minmaxlog':
            #     self.y_normalizer = MinMaxTransformer(y_feats_all, log=True)
            #     print('Target features are normalized using minmaxlog transformer')
            #     print(self.y_normalizer.max, self.y_normalizer.min)

            elif self.normalize_y == 'log':
                self.y_normalizer = LogTransformer(y_feats_all)
                print('Target features are normalized using log transformer')
                print(self.y_normalizer.min)

            elif self.normalize_y == 'quantile':
                self.y_normalizer = QuantileTransformer(output_distribution='normal', n_quantiles=1000)
                self.y_normalizer = self.y_normalizer.fit(y_feats_all)
                self.y_normalizer = TorchQuantileTransformer(self.y_normalizer.output_distribution, self.y_normalizer.references_,self.y_normalizer.quantiles_)
                print('Target features are normalized using quantile transformer')
        ...
        for g in self.graphs:
            # print('y max before transform: ', g.ndata["y"].max(dim=0)[0])
            y_ = self.y_normalizer.transform(g.ndata["y"][:, :-self.sens_dim], inverse=False)  # a torch quantile transformer
            g.ndata['y'] = torch.cat([y_, g.ndata['y'][:, -self.sens_dim:]], dim=-1)
            # print('y max after transform: ', g.ndata["y"].max(dim=0)[0])

        # print('Target features are normalized using quantile transformer')
        # print('Target features are normalized using unit transformer')
        
''' 
    DataLoader for reference network, composes of a pair of data:
    reference data: f2 = (x2, y2) is appended in the tuple
    Given two data [x1, y1, ..., geo1] and [x2, y2, ..., geo2],
    return paired data: [x1, y1, ..., (f2, geo1, geo2, ...)]
'''
class RefDataLoader(torch.utils.data.DataLoader):
    def __init__(self, dataset, batch_size=1,sort_data=True, shuffle=False, sampler=None,
                 batch_sampler=None, num_workers=0, collate_fn=None,
                 pin_memory=False, drop_last=False, timeout=0,
                 worker_init_fn=None):
        super(RefDataLoader, self).__init__(dataset=dataset, batch_size=batch_size,
        ...
        self.ref_dict = {}           
        self.ref_rel_dist = 0.5 # the maximum relative distance for reference     
        # self.__build_ref_dict()
        self.ns = 2 # neighbor size
    
    # def __build_ref_dict(self):
    #     # Find reference by KNN 
    #     K = 1
    #     # Calculate the distance between each data point in X_test and each data point in X_train.
    #     u_p_list = torch.stack([u_p for _, u_p, _ in self.dataset], dim=0)        
    #     distances = torch.norm(u_p_list[:, None] - u_p_list[None, ...], dim=-1)

    #     # Sort the distances for each data point in X_test.
    #     sorted_distances = distances.argsort(dim=1)

    #     # Return the indices of the k-nearest neighbors of each data point in X_test.
    #     self.ref_dict = {i: sorted_distances[i, :K+1] for i in range(len(self.dataset))}
    #     # print('ref dictionary: ', self.ref_dict)

    #     return

    def relp_dist(self, q, r, p=2):
        error = r - q
    ...
    def ref_idx(self, i):
        # choices = self.ref_dict[i]
        # choices = choices[1:]        
        # ref_idx = np.random.choice(choices)
        # print('ref by KNN: ', i, ref_idx)

        n_step = self.dataset.n_step
        q = i // n_step
        l, r = q * n_step, (q+1) * n_step
        choices = [i]
        for j in range(-self.ns, self.ns+1):
            if j==0: continue # Remove self reference
            idx = i + j
            
            if idx >= l and idx < r:
                query = self.dataset[i][0].ndata['y']
                ref = self.dataset[idx][0].ndata['y']
                if self.relp_dist(query, ref) < self.ref_rel_dist:
                    choices.append(idx)

        ref_idx = np.random.choice(choices)
        # print(i, ref_idx, choices, self.ref_rel_dist)
        return ref_idx
    ...
    def __iter__(self):
        ...
        for indices in self.batch_indices:    
            ...
            transposed_ref_ = zip(*[self.dataset[self.ref_idx(idx)] for idx in indices])
            transposed_ref = [s for s in transposed_ref_]                                 ###TODO: memory to be optimized
            batched = []
            for i, sample in enumerate(transposed):
                if i==0:
                    assert isinstance(sample[0], dgl.DGLGraph)
                    batched.append(dgl.batch(list(sample)))
                    sample_ref = transposed_ref[0]
                    batched.append(dgl.batch(list(sample_ref)))
                elif i==1:
                    assert isinstance(sample[0], torch.Tensor)
                    sample = torch.stack(sample)
                    sample_r = torch.stack(transposed_ref[1])
                    sample = torch.cat([sample, sample_r], dim=-1)
                    batched.append(sample)
                elif i==2:
                    assert isinstance(sample[0], MultipleTensors) or isinstance(sample[0], torch.Tensor)
                    # X_Y_ref = [pad_sequence([torch.cat([g.ndata['x'], g.ndata['y']], dim=-1) for g in transposed_ref[0]]).permute(1, 0, 2)] # B, T, F   
                    geo_ref, geo = [], []
                    if isinstance(sample[0], MultipleTensors):
                        geo_ref = [pad_sequence([transposed_ref[-1][i][j] for i in range(len(transposed_ref[-1]))]).permute(1, 0, 2) for j in range(len(transposed_ref[-1][0]))] 
                        geo = [pad_sequence([sample[i][j] for i in range(len(sample))]).permute(1, 0, 2) for j in range(len(sample[0]))]                 
                    # sample_pair = X_Y_ref + geo_ref + geo
                    sample_pair = geo_ref + geo
                    sample_pair = MultipleTensors(sample_pair)
                    batched.append(sample_pair)
                else:
                    raise NotImplementedError
...
class WeightedLpRelLoss(_WeightedLoss):
    ...
    def _lp_losses(self, g, pred, target):
        ...
        # loss = losses.mean()
        # return loss, metrics
        return losses.mean(dim=0), metrics
    ...
    def forward(self, g,  pred, target):
        ...
        losses, metrics = self._lp_losses(g, pred, target)
        # print('losses before normalizer: ', losses)
        # print('metrics before normalizer: ', metrics)
        ...
        if self.normalizer is not None:
            ...
            # print(ori_pred.shape, ori_target.shape)
            # print(ori_pred.abs().max(dim=0)[0], ori_target.abs().max(dim=0)[0])
            p = 2
            r, q = ori_pred, ori_target
            error = r - q
            rel_dist = ((error.abs()**p).sum(dim=0) / (q.abs()**p).sum(dim=0)) ** (1/p)
            # print('relative error: ', rel_dist)
            # print((ori_pred - ori_target).abs().sum(dim=0), ori_target.abs().sum(dim=0))


        if self.regularizer:
            raise NotImplementedError
        else:
            reg = torch.zeros_like(losses.mean())

        # print('losses: ', losses)
        # print('metrics: ', metrics)
        return losses, reg, metrics
